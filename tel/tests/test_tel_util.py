...
class TestTwilioFindNumberInAreaCode:
    ...
    def test_find_number_with_area_code_returns_results(self, mock_twilio_client, mock_phone_number):
        ...
        mock_twilio_client.available_phone_numbers.return_value.local.list.assert_called_once_with(
            area_code='415', limit=20
        )
...
class TestTwilioBuyPhoneNumber:
    ...
    def test_buy_number_with_hipaa_enabled_creates_subaccount_client(self, mock_client_class, mock_twilio_client, mock_phone_number):
        hipaa_config = {
            'enabled': True,
            'subaccount_sid': 'AC_subaccount',
            'subaccount_token': 'sub_token',
            'subaccount_twiml_sid': 'AP_twiml_sub'
        }
        ...
        mock_subaccount_client.incoming_phone_numbers.create.assert_called_once_with(
            phone_number=mock_phone_number.phone_number,
            sms_application_sid='AP_twiml_sub',
            voice_application_sid='AP_twiml_sub'
        )
    ...
    def test_buy_specific_phone_number_success(self, mock_cleanup, mock_twilio_client, mock_phone_number):
        ...
        mock_twilio_client.incoming_phone_numbers.create.assert_called_once_with(
            sms_application_sid='AP_app_sid',
            voice_application_sid='AP_app_sid',
    ...
    def test_buy_number_exhausts_retries_raises_error(self, mock_twilio_client, mock_phone_number):
        mock_twilio_client.available_phone_numbers.return_value.local.list.return_value = [mock_phone_number]
        mock_twilio_client.incoming_phone_numbers.create.side_effect = TwilioRestException(
            status=400, uri='/test', msg='Error'
        )
...
class TestTwilioAPIHandlerCreateMsgService:
    ...
    def test_create_msg_service_success(self, twilio_handler):
        ...
        twilio_handler.twilio_client.messaging.services.create.assert_called_once_with(
...
class TestTwilioAPIHandlerCreatePhoneNumberServiceAssociation:
    ...
    def test_create_association_success(self, twilio_handler):
        ...
        twilio_handler.twilio_client.messaging.services.return_value.phone_numbers.create.assert_called_once_with(
...
class TestTwilioAPIHandlerDeleteTnAssociation:
    ...
    def test_delete_association_twilio_exception_returns_false(self, twilio_handler):
        twilio_handler.twilio_client.messaging.services.return_value.phone_numbers.return_value.delete.side_effect = TwilioRestException(
...
class TestTwilioAPIHandlerCreateExternalCampaign:
    ...
    def test_create_campaign_success(self, twilio_handler):
        ...
        assert result is True
        twilio_handler.twilio_client.messaging.external_campaign.create.assert_called_once_with(
            campaign_id='campaign_123',
            messaging_service_sid='MG_sid'
        )

    def test_create_campaign_twilio_exception_returns_false(self, twilio_handler):
        twilio_handler.twilio_client.messaging.external_campaign.create.side_effect = TwilioRestException(
            status=400, uri='/test', msg='Error'
        )
...
class TestTwilioAPIHandlerGetActiveSubaccounts:
    ...
    def test_get_active_subaccounts_success(self, twilio_handler, mock_settings):
        ...
        mock_settings.TWILIO_CLIENT.api.v2010.accounts.list.assert_called_once_with(
            friendly_name='TestAccount',
            status='active'
        )
...
class TestTwilioAPIHandlerCloseSubAccount:
    ...
    def test_close_subaccount_twilio_exception_returns_false(self, twilio_handler):
        twilio_handler.twilio_client.api.v2010.accounts.return_value.update.side_effect = TwilioRestException(
            status=404, uri='/test', msg='Not found'
        )
...
class TestTwilioAPIHandlerDcaPartnerElection:
    ...
    def test_dca_partner_election_success_with_accepted_status(self, mock_json_loads, mock_search_users, twilio_handler):
        with patch('..twilio_util.TwilioAPIHandler.create_twilio_msg_service') as mock_create_service, \
            ...
            with patch('..twilio_util.EnterpriseAccount') as mock_enterprise, \
                 patch('..twilio_util.TcrMessaging') as mock_tcr, \
                 patch('..twilio_util.Campaign') as mock_campaign_class:
