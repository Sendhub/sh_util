...
def send_email(subject, body, from_address, to_address):
    ...
    """
    logging.info(
        "Sending email, subject=%s, body=%s, from_address=%s, to_address=%s",  # noqa
        subject,
        body,
        from_address,
        to_address,
...
def send_error_email(error, request=None):
    ...
    message = "{}\n\n{}".format(stacktrace, request_repr)

    settings.DEBUG = False

    if not hasattr(settings, "DEBUG") or (hasattr(settings, "DEBUG") and not settings.DEBUG):
        from django.core.mail import mail_admins

        if request is not None:
            subject = "SendHub Exception (%s IP): %s" % ((request.META.get("REMOTE_ADDR") in settings.INTERNAL_IPS and "internal" or "EXTERNAL"), request.path)
        else:
...
__all__ = ["send_email", "send_html_email", "send_error_email"]
